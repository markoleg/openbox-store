...
import { issueReviewContext, reviewCommandsEnabled } from '@/lib/server/reviewCommands';
...
export async function POST(req: NextRequest) {
  ...
  try {
    return NextResponse.json(await issueReviewContext(context), {headers:{'Cache-Control':'no-store'}});
  } catch (error) {
